	...
	function genPaginator(data,status){
		...
		var options = {
		        ...
		        itemTexts: function(type, page, current) { //修改显示文字
	            	...
	            	if(isSearch == 0)
	            		{
	            			ThinkAjax.send("<?php echo U('Admin/getRouterList');?>",'ajax=1&PageSize=10&PageNum='+page,completeRouterList,'');
	            		}else
	            			{
	            				$('PageNum').value = page;
...
<script  language="JavaScript">
	...
	//开始构造分页插件
	ThinkAjax.send("<?php echo U('Admin/getRouterList');?>",'ajax=1&PageSize=10&PageNum=1',genPaginator,'');
