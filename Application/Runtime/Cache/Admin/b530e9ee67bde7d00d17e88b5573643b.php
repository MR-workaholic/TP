...
//构造分页器
	function genPaginator(data,status){
		...
		var options = {
		        ...
		        itemTexts: function(type, page, current) { //修改显示文字
	            	...
	            	if(data['isSearch'] == 0)
	            		{
	            			ThinkAjax.send("<?php echo U('AgentMessage/getRouterList');?>",'ajax=1&PageSize='+mPageSize+'&PageNum='+page,completeRouterList,'');
	            		}else{
	            			$('PageNum').value = page;
		...
		if(status == 0)
			...
			tbodies[0].innerHTML = newtbodies;
				...
				for(var i=0;i<routerList.length;i++){
					...
					newtbodies += "<td><a href=\"javascript:\" onclick=\"changeAD('"+routerList[i]['Mac']+"')\">更换</a></td></tr>";
				}
				tbodies[0].innerHTML=newtbodies;
	...
	function getRouterDetail(routerId){
		...
		window.open(url,"","width=1000,height=200,top="+top+",left="+left+",resizable=no");
	} 
	...
	function changeAD(mac)
	{
		alert(mac);
		var top = document.body.clientHeight / 4;
		var left = document.body.clientWidth / 4; 
		window.open("../AgentMessage/showChangeAD/mac/"+mac, "", "width=700,height=400,top="+top+",left="+left+",resizable=no");
	}
...
var mPageSize = parseInt(document.body.clientHeight / 100) - 2;
$('PageSize').value = mPageSize;
      
//开始构造分页插件
 ThinkAjax.send("<?php echo U('AgentMessage/getRouterList');?>",'ajax=1&PageNum=1&PageSize='+mPageSize, genPaginator,'');
