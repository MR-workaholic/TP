...
<head>
    ...
    function updatathemebasic()
    {
    	...
    	var addefault = jq("input[name='defaultTheme']:checked").val();
    	ThinkAjax.send("<?php echo U('Adset/updatathemebasic');?>",'ajax=1&aid=<?php echo ($aid); ?>&adname='+adname+'&adstatus='+adstatus+'&adremark='+adremark+'&addefault='+addefault, completeupdatathemebasic,'');
    }
...
<div class="theme-basic">
  <ul>
    ...
    <li><span>默认主题：</span><input id="tip1" type="button" class="mytip" value="提示"/><br/>
      <input type="radio" name="defaultTheme"/><label>设为默认主题</label>
    </li>
  ...
  <input type="button" class="btn" name="cancelTheme-basic" onclick="updatathemebasic()" value="保存"></div>
  <p class="tip" id="1">当新设备接入时，使用该主题作为新设备的主题。默认主题的状态永远都是开启的！</p>
...
<script>
	...
	var addefault = "<?php echo ($addefault); ?>";
	...
	if(addefault == 'Y')
	{
		 jq("[name = defaultTheme]:radio").attr("checked", true);
	}
