...
!function (a, b) {
	...
	e.DEFAULTS = {selector: "li, div", dragCssClass: "invisible"}, e.prototype.getOptions = function (b) {
		...
		function c(b) {
			...
			var e = 0;
			b.each(function () {
				a(this).attr("data-order", c[e++])
			})
		}

		var d = this.$, e = this.options, f = e.reverse;
		c(b), b.droppable({
			trigger: e.trigger,
			...
			drag: function (b) {
				if (d.addClass("sortable-sorting"), b.isIn) {
					var g = b.element, h = b.target, i = g.attr("data-order"), j = h.attr("data-order");
					if (i == j)return;
					i > j ? h[f ? "after" : "before"](g) : h[f ? "before" : "after"](g);
					var k = d.children(e.selector).not(".drag-shadow");
					c(k), a.zui.callEvent(e.order, {list: k, element: g})
				}
			},
			finish: function (b) {
	...
	d.VERSION = "3.2.0", d.TRANSITION_DURATION = 300, d.BACKDROP_TRANSITION_DURATION = 150, d.DEFAULTS = {
		...
		f.$element.trigger(g), f.isShown || g.isDefaultPrevented() || (f.isShown = !0, f.options.moveable && f.setMoveale(), f.checkScrollbar(), f.$body.addClass("modal-open"), f.setScrollbar(), f.escape(), f.$element.on("click.dismiss." + c, '[data-dismiss="modal"]', a.proxy(f.hide, f)), f.backdrop(function () {
	...
	g.DEFAULTS = {
		...
		var p = function (a) {
			"undefined" == typeof a && (a = 300), setTimeout(function () {
				k = j.find(".modal-dialog"), h.width && "auto" != h.width && k.css("width", h.width),
				h.height && "auto" != h.height && (k.css("height", h.height), "iframe" === h.type && m.css("height", k.height() - n.outerHeight())), i.ajustPosition(h.position), j.removeClass("modal-loading"), "iframe" != h.type && k.off("resize." + c).on("resize." + c, function () {
					i.ajustPosition()
				})
			}, a)
		};
		if ("custom" === h.type && l)if (a.isFunction(l)) {
			...
			var r = "iframe-" + h.name;
			n.detach(), m.detach(), o.empty().append(n).append(m), m.css("padding", 0).html('<iframe id="' + r + '" name="' + r + '" src="' + h.url + '" frameborder="no"  allowfullscreen="true" mozallowfullscreen="true" webkitallowfullscreen="true"  allowtransparency="true" scrolling="auto" style="width: 100%; height: 100%; left: 0px;"></iframe>'), h.waittime > 0 && (i.waitTimeout = setTimeout(p, h.waittime));
		...
		j.modal({show: "show", backdrop: h.backdrop, moveable: h.moveable, keyboard: h.keyboard})
	...
	b.DEFAULTS = {auto: !1, foldicon: "icon-chevron-right"}, b.prototype.getOptions = function (c) {
		...
		this.$.on("click", ".nav-parent > a", function (d) {
			...
			return e.hasClass("show") ? (e.find(".icon-rotate-90").removeClass("icon-rotate-90"), e.find(".nav").slideUp(function () {
				...
				//a(this).closest(".nav-parent").addClass("active");
			})), d.preventDefault(), !1
		})
