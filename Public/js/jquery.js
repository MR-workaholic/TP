...
(function( global, factory ) {
	...
	if ( typeof module === "object" && typeof module.exports === "object" ) {
		...
		// execute the factory and get $
		// For environments that do not inherently posses a window with a document
		// (such as Node.js), expose a $-making factory as module.exports
 ...
 * http://sizzlejs.com/
 *
 * Copyright 2008, 2014 $ Foundation, Inc. and other contributors
 * Released under the MIT license
 * http://jquery.org/license
 *
 * Date: 2014-12-16
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// IE will throw "permission denied" error when accessing "document" variable, see $ #13936
...
$.filter = function( expr, elems, not ) {
	...
	return elems.length === 1 && elem.nodeType === 1 ?
		$.find.matchesSelector( elem, expr ) ? [ elem ] : [] :
		$.find.matches( expr, $.grep( elems, function( elem ) {
...
$.fn.extend({
	find: function( selector ) {
		...
		ret = this.pushStack( len > 1 ? $.unique( ret ) : ret );
			...
			if ( match && (match[1] || !context) ) {
				...
				// HANDLE: $(#id)
					...
					if ( elem && elem.parentNode ) {
						...
						// Otherwise, we inject the element directly into the $ object
						this.length = 1;
						this[0] = elem;
					}
...
$.fn.extend({
	...
	closest: function( selectors, context ) {
		...
		return this.pushStack( matched.length > 1 ? $.unique( matched ) : matched );
	},
...
$.acceptData = function( elem ) {
	var noData = $.noData[ (elem.nodeName + " ").toLowerCase() ],
...
function internalData( elem, name, data, pvt /* Internal Use Only */ ) {
	...
	if ( !id ) {
		...
		if ( isNode ) {
			id = elem[ internalKey ] = deletedIds.pop() || $.guid++;
		} else {
			id = internalKey;
		}
	}
	...
	if ( !cache[ id ] ) {
		// Avoid exposing $ metadata on plain JS objects when the object
...
function internalRemoveData( elem, name, pvt ) {
	...
	var thisCache, i,
		...
		cache = isNode ? $.cache : elem,
		id = isNode ? elem[ $.expando ] : $.expando;
	...
	if ( name ) {
		...
		if ( thisCache ) {
			...
			if ( pvt ? !isEmptyDataObject(thisCache) : !$.isEmptyObject(thisCache) ) {
				return;
			}
		}
	}
...
$.extend({
	...
	hasData: function( elem ) {
		elem = elem.nodeType ? $.cache[ elem[$.expando] ] : elem[ $.expando ];
...
$.fn.extend({
	data: function( key, value ) {
		...
		// Special expections of .data basically thwart $.access,
		...
		if ( key === undefined ) {
			if ( this.length ) {
				...
				if ( elem.nodeType === 1 && !$._data( elem, "parsedAttrs" ) ) {
					i = attrs.length;
					while ( i-- ) {
		...
		return arguments.length > 1 ?
			...
			elem ? dataAttr( elem, key, $.data( elem, key ) ) : undefined;
	},
...
$.fn.extend({
	...
	promise: function( type, obj ) {
		...
		while ( i-- ) {
			tmp = $._data( elements[ i ], type + "queueHooks" );
			if ( tmp && tmp.empty ) {
				count++;
...
var isHidden = function( elem, el ) {
		// isHidden might be called from $#filter function;
...
var access = $.access = function( elems, fn, key, value, chainable, emptyGet, raw ) {
	var i = 0,
		length = elems.length,
		bulk = key == null;
...
$.event = {
	...
	remove: function( elem, types, handler, selector, mappedTypes ) {
		var j, handleObj, tmp,
			...
			namespaces, origType,
			elemData = $.hasData( elem ) && $._data( elem );
	...
	$.event.special.submit = {
		setup: function() {
			...
			$.event.add( this, "click._submit keypress._submit", function( e ) {
				...
				if ( form && !$._data( form, "submitBubbles" ) ) {
					$.event.add( form, "submit._submit", function( event ) {
						event._submit_bubble = true;
					});
					$._data( form, "submitBubbles", true );
				}
			});
		...
		setup: function() {
			...
			if ( rformElems.test( this.nodeName ) ) {
				...
				if ( this.type === "checkbox" || this.type === "radio" ) {
					$.event.add( this, "propertychange._change", function( event ) {
						if ( event.originalEvent.propertyName === "checked" ) {
							this._just_changed = true;
						}
					});
					$.event.add( this, "click._change", function( event ) {
			...
			$.event.add( this, "beforeactivate._change", function( e ) {
				...
				if ( rformElems.test( elem.nodeName ) && !$._data( elem, "changeBubbles" ) ) {
					$.event.add( elem, "change._change", function( event ) {
						if ( this.parentNode && !event.isSimulated && !event.isTrigger ) {
							$.event.simulate( "change", this.parentNode, event, true );
						}
					});
					$._data( elem, "changeBubbles", true );
				}
			});
		},
...
if ( !support.focusinBubbles ) {
	$.each({ focus: "focusin", blur: "focusout" }, function( orig, fix ) {
		...
		var handler = function( event ) {
				$.event.simulate( fix, event.target, $.event.fix( event ), true );
			};

		$.event.special[ fix ] = {
			setup: function() {
				var doc = this.ownerDocument || this,
					attaches = $._data( doc, fix );

				if ( !attaches ) {
					doc.addEventListener( orig, handler, true );
				}
				$._data( doc, fix, ( attaches || 0 ) + 1 );
			},
			teardown: function() {
				var doc = this.ownerDocument || this,
					attaches = $._data( doc, fix ) - 1;

				if ( !attaches ) {
					doc.removeEventListener( orig, handler, true );
					$._removeData( doc, fix );
				} else {
					$._data( doc, fix, attaches );
				}
			}
		};
	});
...
function disableScript( elem ) {
	elem.type = ($.find.attr( elem, "type" ) !== null) + "/" + elem.type;
	return elem;
}
...
function setGlobalEval( elems, refElements ) {
	...
	for ( ; (elem = elems[i]) != null; i++ ) {
		$._data( elem, "globalEval", !refElements || $._data( refElements[i], "globalEval" ) );
	}
}
...
function fixCloneNodeIssues( src, dest ) {
	...
	if ( !support.noCloneEvent && dest[ $.expando ] ) {
		data = $._data( dest );

		for ( e in data.events ) {
			$.removeEvent( dest, e, data.handle );
		}
	...
	// IE10 throws NoModificationAllowedError if parent is null, #12132.
		...
		if ( support.html5Clone && ( src.innerHTML && !$.trim(dest.innerHTML) ) ) {
...
$.extend({
	clone: function( elem, dataAndEvents, deepDataAndEvents ) {
		var destElements, node, clone, i, srcElements,
			inPage = $.contains( elem.ownerDocument, elem );

		if ( support.html5Clone || $.isXMLDoc(elem) || !rnoshimcache.test( "<" + elem.nodeName + ">" ) ) {
		...
		if ( (!support.noCloneEvent || !support.noCloneChecked) &&
				(elem.nodeType === 1 || elem.nodeType === 11) && !$.isXMLDoc(elem) ) {
			...
			if ( elem || elem === 0 ) {
				...
				// Convert html into DOM nodes
					...
					if ( !support.tbody ) {
						...
						while ( j-- ) {
							if ( $.nodeName( (tbody = elem.childNodes[j]), "tbody" ) && !tbody.childNodes.length ) {
		...
		for ( ; (elem = elems[i]) != null; i++ ) {
			if ( acceptData || $.acceptData( elem ) ) {
				...
				if ( data ) {
					...
					// Remove cache only if it was not already removed by $.event.remove
					if ( cache[ id ] ) {
...
$.fn.extend({
	...
	remove: function( selector, keepData /* Internal Use Only */ ) {
		...
		for ( ; (elem = elems[i]) != null; i++ ) {
			...
			if ( elem.parentNode ) {
				if ( keepData && $.contains( elem.ownerDocument, elem ) ) {
	...
	empty: function() {
		...
		for ( ; (elem = this[i]) != null; i++ ) {
			...
			// Support: IE<9
			if ( elem.options && $.nodeName( elem, "select" ) ) {
				...
				if ( hasScripts ) {
					...
					for ( i = 0; i < hasScripts; i++ ) {
						...
						if ( rscriptType.test( node.type || "" ) &&
							!$._data( node, "globalEval" ) && $.contains( doc, node ) ) {
							...
							if ( node.src ) {
								...
								$.globalEval( ( node.text || node.textContent || node.innerHTML || "" ).replace( rcleanScript, "" ) );
							}
						}
					}
				}
...
$.each({
	...
	$.fn[ name ] = function( selector ) {
		...
		for ( ; i <= last; i++ ) {
			...
			// Modern browsers can apply $ collections as arrays, but oldIE needs a .get()
...
function defaultDisplay( nodeName ) {
	...
	if ( !display ) {
		...
		if ( display === "none" || !display ) {
			...
			iframe = (iframe || $( "<iframe frameborder='0' width='0' height='0'/>" )).appendTo( doc.documentElement );
	...
	for ( ; index < length; index++ ) {
		...
		if ( show ) {
			...
			if ( elem.style.display === "" && isHidden( elem ) ) {
				values[ index ] = $._data( elem, "olddisplay", defaultDisplay(elem.nodeName) );
			}
		} else {
			hidden = isHidden( elem );

			if ( display && display !== "none" || !hidden ) {
				$._data( elem, "olddisplay", hidden ? display : $.css( elem, "display" ) );
			}
		}
	}
...
Tween.propHooks = {
	_default: {
		...
		set: function( tween ) {
			...
			} else if ( tween.elem.style && ( tween.elem.style[ $.cssProps[ tween.prop ] ] != null || $.cssHooks[ tween.prop ] ) ) {
	...
	tweeners = {
		"*": [ function( prop, value ) {
			var tween = this.createTween( prop, value ),
				...
				unit = parts && parts[ 3 ] || ( $.cssNumber[ prop ] ? "" : "px" ),

				// Starting value computation is required for potential unit mismatches
				start = ( $.cssNumber[ prop ] || unit !== "px" && +target ) &&
					rfxnum.exec( $.css( tween.elem, prop ) ),
				scale = 1,
				maxIterations = 20;
			...
			if ( start && start[ 3 ] !== unit ) {
				...
				do {
					...
					// Adjust and apply
					start = start / scale;
					$.style( tween.elem, prop, start + unit );
		...
		select: {
			...
			set: function( elem, value ) {
				...
				while ( i-- ) {
					...
					if ( $.inArray( $.valHooks.option.get( option ), values ) >= 0 ) {
	...
	attr: function( elem, name, value ) {
		...
		if ( nType !== 1 || !$.isXMLDoc( elem ) ) {
			...
			hooks = $.attrHooks[ name ] ||
				( $.expr.match.bool.test( name ) ? boolHook : nodeHook );
		}
...
boolHook = {
	set: function( elem, value, name ) {
		if ( value === false ) {
			...
			elem.setAttribute( !getSetAttribute && $.propFix[ name ] || name, name );
...
$.each( $.expr.match.bool.source.match( /\w+/g ), function( i, name ) {
	...
	attrHandle[ name ] = getSetInput && getSetAttribute || !ruseDefault.test( name ) ?
		...
		function( elem, name, isXML ) {
			if ( !isXML ) {
				return elem[ $.camelCase( "default-" + name ) ] ?
					name.toLowerCase() :
					null;
			}
		};
	...
	toggleClass: function( value, stateVal ) {
		...
		return this.each(function() {
			...
			// Toggle whole class name
				...
				this.className = this.className || value === false ? "" : $._data( this, "__className__" ) || "";
			}
		});
	},
...
$.extend({
	...
	ajax: function( url, options ) {
		...
		s.dataTypes = $.trim( s.dataType || "*" ).toLowerCase().match( rnotwhite ) || [ "" ];
...
$.expr.filters.hidden = function( elem ) {
	...
	return elem.offsetWidth <= 0 && elem.offsetHeight <= 0 ||
		(!support.reliableHiddenOffsets() &&
			((elem.style && elem.style.display) || $.css( elem, "display" )) === "none");
};
...
$.ajaxSettings.xhr = window.ActiveXObject !== undefined ?
	// Support: IE6+
	function() {
		...
		if ( !options.crossDomain || support.cors ) {
			...
			return {
				send: function( headers, complete ) {
					...
					// handled in $.ajax (so no try/catch here)
...
$.each( [ "ajaxStart", "ajaxStop", "ajaxComplete", "ajaxError", "ajaxSuccess", "ajaxSend" ], function( i, type ) {
	$.fn[ type ] = function( fn ) {
...
// Expose $ and $ identifiers, even in
